...
// If max lines greater than 0 show chat. If not chat disabled
if ($MaxLines > 0) {
...
/*if (strtoupper($Chatter) == "ADMIN:MASTER") {
        ...
        $PlayerData->Y = 266;
        $PlayerData->MapID = 1;
} else*/if ($Chatter != "" and strlen($Chatter) <= 240) {
	...
	if (preg_match("/^\/(?:msg|tell|t|m) (.+?),(.+)/",$Chatter,$Matches) == 1) {
		...
		$sth = mysql_query($sql);
		if (mysql_num_rows($sth) == 0) { $TargetID == 0; } else { list($TargetID) = mysql_fetch_row($sth); $Chatter = $Matches[1]; }
	...
	if ($Chatter != "") {
		if ($TargetID != 0) $sth = mysql_query("update user set LastTell=$TargetID where CoreID=$PlayerData->CoreID");
...
print "<tr>";
print "<td class=pageCell colspan=3>";
...
if ($PlayerData->ChatType == "clan") {
	...
	if ($ChatCount < $MaxLines) { $ChatCount = $MaxLines; }
...
// Shows NEWS, Clock, and PM bubble, Allows players to not see blocked people. Emotes
print mysql_error();
		...
		if (mysql_num_rows($BlockSTH) > 0) {
			...
			if ($PlayerData->Subscriber == 'Y') {
				$Message = smilies($Message);
                                        ...
                                        if ($ChatTargetID != 0) {
                                                if ($ChatTargetID == $PlayerData->CoreID) { print "[<FONT COLOR=RED>INCOMING PRIV</FONT>] "; }
...
// Chat in 3rd person example: *Username likes cookies
...
print "<TR>\n";
print "	<TD CLASS=Menu COLSPAN=3>";
print "Send Message";
print "</TD>\n";
print "</TR>\n";
print "<TR>\n";
print " <FORM NAME=chatter ACTION=$SCRIPT_NAME>";
print "	<TD COLSPAN=4 VALIGN=TOP class=pageCell>";
